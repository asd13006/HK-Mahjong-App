...
function init() {
    ...
    attachFastClick(
        ...
        () => {
            document.getElementById('conditionsIsland').classList.toggle('expanded');
    ...
    attachFastClick(
        ...
        () => {
            if (confirm('⚠️ 確定要清空所有生涯戰績嗎？這個動作無法復原喔！')) {
                localStorage.removeItem('mahjongHistory');
                renderHistory();
                updateProfileData();
            }
        },
                ...
                () => {
                    if (activeConditions.has(cond.id)) {
                        ...
                        if (cond.id === 'kongSelfDrawn' || cond.id === 'doubleKongSelfDrawn')
                            activeConditions.add('selfDrawn');
...
function getTileInfo(id) {
    if (id < 9) return { type: 'w', num: id + 1, suit: '萬' };
    if (id < 18) return { type: 't', num: id - 8, suit: '筒' };
    if (id < 27) return { type: 's', num: id - 17, suit: '索' };
    const zNames = ['東', '南', '西', '北', '中', '發', '白'];
    return { type: 'z', num: zNames[id - 27], suit: '' };
}
...
function resetResultCard() {
    ...
    document.getElementById('heroMainPatternName').style.color = '#10b981';
...
function renderHand() {
    ...
    const updateGridDOM = () => {
        ...
        existingTiles.forEach((el, key) => {
            if (!hand.find((t) => String(t.key) === key)) el.remove();
        });
...
function isNineGates(counts) {
    ...
    for (let s = 0; s < 3; s++) {
        let start = s * 9;
        let suitSum = 0;
        for (let i = 0; i < 9; i++) suitSum += counts[start + i];
        if (suitSum === 14) {
            let base = [3, 1, 1, 1, 1, 1, 1, 1, 3];
            let isValid = true;
            for (let i = 0; i < 9; i++) if (counts[start + i] < base[i]) isValid = false;
...
function evaluateStandardPatterns(breakdown, counts) {
    ...
    for (let i = 0; i < 34; i++) {
        if (counts[i] > 0) {
            if (i < 9) suits.add('w');
            else if (i < 18) suits.add('t');
            else if (i < 27) suits.add('s');
            else hasZ = true;
        }
    }
    ...
    if (isAllPongs) {
        ...
        for (let i = 0; i < 34; i++) {
            if (counts[i] > 0 && i < 27 && i % 9 !== 0 && i % 9 !== 8) isAllTerminals = false;
        }
    ...
    if (wPongs === 4) {
        ...
        if (!isExceptional) {
            let roundWindId = 27 + roundWind;
            let seatWindId = 27 + seatWind;
...
function displayResult(faan, tags, isWin) {
    ...
    let mainName = '平糊';
    if (isZaWu) {
        heroCard.classList.add('glow-fail');
        badge.innerText = '🚨 判定失敗';
        badge.classList.add('fail');
        scoreVal.innerText = '--';
        scoreUnit.style.display = 'none';
        mainPattern.innerText = '詐糊';
        mainPattern.style.color = '#ef4444';
        if (navigator.vibrate) navigator.vibrate([50, 50, 50]);
    } else {
        heroCard.classList.add('glow-normal');
        ...
        if (isBaauPang) {
            ...
            badge.innerText = '✨ 極限爆棚';
            badge.classList.add('legendary');
        } else if (faan >= 7) {
            badge.innerText = '🔮 史詩大牌';
            badge.classList.add('epic');
        } else {
            badge.innerText = '✅ 結算成功';
            badge.classList.add('epic');
        }
        let maxFaan = -1;
        tags.forEach((t) => {
        ...
        mainPattern.style.color = '#10b981';
    ...
    tags.forEach((t, index) => {
        ...
        let icon = '✦';
        if (name.includes('花')) icon = '🌸';
        else if (name.includes('圈') || name.includes('位')) icon = '💨';
        item.innerHTML = `<div class="detail-item-name"><div class="detail-item-icon">${icon}</div>${name}</div><div class="detail-item-score" style="${isZaWu ? 'color: #ef4444; background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.2);' : ''}">${scoreText}</div>`;
...
function openWikiDetail(item) {
    ...
    document.getElementById('wdDesc').innerHTML =
        item.desc || `<strong style='color:#10b981; font-size:16px;'>${item.name}</strong><br><br>${item.d}`;
...
function updateProfileData() {
    ...
    if (levelEl) {
        ...
        levelEl.innerText = `🏆 ${title} LV.${level}`;
    }
