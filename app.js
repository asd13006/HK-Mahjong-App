...
function init() {
    ...
    attachFastClick(document.getElementById('islandHeaderBtn'), () => {
        ...
        // ✨ 優化方案 A (開始)：動畫啟動的瞬間，套用「無毛玻璃替身」
        island.classList.add('animating-no-blur');

        // 執行原本的展開/收合動作
        island.classList.toggle('expanded');
        if (navigator.vibrate) navigator.vibrate([5]);

        // ✨ 優化方案 A (結束)：設定 500 毫秒 (剛好是 CSS 動畫結束的時間)
        // 時間一到，立刻把「無毛玻璃替身」拿掉，華麗的毛玻璃瞬間回歸！
        setTimeout(() => {
            island.classList.remove('animating-no-blur');
        }, 500);
    }, 'is-tapped-island'); document.querySelectorAll('#roundWindSelector .wind-tab').forEach((tab, i) => attachFastClick(tab, () => setRoundWind(i), 'is-tapped-chip'));
...
function renderKeyboard() {
    ...
    TILE_DEFS.forEach(def => {
        ...
        for (let i = 0; i < limit; i++) {
            ...
            btn.dataset.id = id; /* ✨ 新增這行：把牌的 ID 綁定在 HTML 標籤上 */
...
function updateKeyboardState() {
    ...
    document.querySelectorAll('#keyboard .tile').forEach(btn => {
        ...
        if (counts[id] >= 4) {
            btn.classList.add('disabled-tile');
        } else {
            btn.classList.remove('disabled-tile');
        }
    });
}
